...
all.models <- c("Ridge", "LASSO-Min", "LASSO-1se", "LS", "hybrid stepwise", "PLS", "RTBoot", "RF", "RF_Tune", )
...
for (i in 1:K) {
  ...
  ### Regression Tree (RT), with boosting
...
```
Very similar, but hybrid stepwise has lowest MSPE.

Next up is random forest.
```{r}
library(randomForest)

all.models2 <- c("RF", "Boosting")
all.MSPEs2 <- array(0, dim = c(K, length(all.models2)))
colnames(all.MSPEs2) <- all.models2

fit.rf.1 <- randomForest(Y ~ ., data = proj, importance = T)
...
(OOB.MSPE.1 <- get.MSPE(proj$Y, OOB.pred.1))
