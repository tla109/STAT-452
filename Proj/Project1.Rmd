...
all.models <- c("Ridge", "LASSO-Min", "LASSO-1se", "LS", "hybrid stepwise", "PLS", "RTBoot")
...
Next up is regression tree.
```{r}
library(rpart)

all.models2 <- c("RTBoot", "RF", "Boosting")
...
for (i in 1:K) {
  ...
  MSPE.RTBoot.min <- get.MSPE(Y.valid, pred.tree)
  all.MSPEs2[i, "RTBoot"] <- MSPE.RTBoot.min
