...
library(dplyr)
data <- select(
  data.in, residual.sugar, density, pH, sulphates,
  alcohol
)
head(data)
...
### input.
### if you run into issues running this code on a mac, feel free to skip
### this part
...
### values we just made
pred.grid <- data.frame(expand.grid(
...
open3d()
persp3d(
  x = vals.density, y = vals.pH, z = pred.alcohol,
  col = "orange"
)
