...
source("Tutorials/Read_Wine_Data_Remove_Duplicates.R")
...
legend("topright",
  title = "Degrees of Freedom", legend = c("4", "8", "12"),
...
legend("topright",
  title = "Degrees of Freedom", legend = c("CV", "GCV"),
  lty = c(2, 3)
)
...
fit.loess.4 <- loess(alcohol ~ density,
  data = data,
  enp.target = 4
)
fit.loess.8 <- loess(alcohol ~ density,
  data = data,
  enp.target = 8
)
fit.loess.12 <- loess(alcohol ~ density,
  data = data,
  enp.target = 12
)
